import{d as f,e as p}from"./chunk-AW5KTZNU.js";import{B as L,C,D as $,G as k,M as y,a as g,c as l,p as M,q as D,r as H}from"./chunk-3BNHFXWM.js";function O(){let{songs:a,streams:t}=p.data,i=t.slice(0,5),s=f(),d=g("#panel-dashboard"),r=H(t,s);d.innerHTML=`
  ...
  `,F()}var j="sh1an-watch-history-v1";function x(){try{return JSON.parse(localStorage.getItem(j)||"[]")}catch{return[]}}function R(a){let t=Math.max(0,Math.floor(a)),i=Math.floor(t/3600),s=Math.floor(t%3600/60),d=t%60;return i>0?`${i}:${String(s).padStart(2,"0")}:${String(d).padStart(2,"0")}`:`${s}:${String(d).padStart(2,"0")}`}function B(){let a=x().slice(0,6);return`
    ...
    </div>`}function F(){let a=g("#dashboard-resume-list");a&&(a.onclick=s=>{let d=s.target.closest("[data-resume-idx]");if(!d)return;let r=x()[Number(d.dataset.resumeIdx)];if(!r?.url)return;let o=null;r.channel!=null&&r.index!=null&&(o=(p.channelData?.combined?.streams||p.data?.streams||[]).find(e=>e.channel===r.channel&&e.index===r.index)||null),window.__openStreamViewer?.(o||{url:r.url,title:r.title,isMv:!!r.isMv},r.t)});let t=g("#dashboard-resume-clear");t&&(t.onclick=()=>{try{localStorage.removeItem(j)}catch{}let s=g("#dashboard-resume-list");s&&(s.innerHTML='<div class="empty-state">\u518D\u751F\u5C65\u6B74\u306F\u307E\u3060\u3042\u308A\u307E\u305B\u3093</div>')});let i=g("#dashboard-resume-queue");i&&(i.onclick=()=>{let s=x(),d=p.channelData?.combined?.streams||p.data?.streams||[],r=s.map((o,n)=>{let e=o.channel!=null&&o.index!=null?d.find(c=>c.channel===o.channel&&c.index===o.index):null;return e?.url?{kind:"stream",key:`${e.channel}:${e.index}`,stream:e}:o.url?{kind:"mv",key:`history:${n}`,video:{url:o.url,title:o.title||"\u52D5\u753B",isMv:!!o.isMv}}:null}).filter(Boolean);r.length&&window.__playMyListInViewer?.({name:"\u8996\u8074\u5C65\u6B74",items:r,idx:0})})}function N(a,t,i){let s=t.filter(n=>n.daysSinceLast>=180).sort((n,e)=>e.count-n.count).slice(0,5),d=t.filter(n=>n.daysSinceLast!=null&&n.daysSinceLast<=30).sort((n,e)=>e.count-n.count).slice(0,5),r=M(a,"month",f()),o=M(a,"year",f());return`
      ...
      <div class="latest-stream-log">
        ...
        <div class="latest-stream-body">
          ...
          <div class="latest-setlist">
            ${i.length?i.map((s,d)=>{let r=s.key?`<button class="latest-sl-title" type="button" data-songkey="${l(s.key)}">${l(s.title||"\u2014")}</button>`:`<span class="latest-sl-title">${l(s.title||"\u2014")}</span>`,o=s.artist?`<button class="latest-sl-artist" type="button" data-artist-search="${l(s.artist)}">${l(s.artist)}</button>`:"";return`<div class="latest-setlist-item"><strong class="latest-sl-num">${d+1}</strong><div class="latest-sl-info">${r}${o}</div></div>`}).join(""):"<span>\u30BB\u30C3\u30C8\u30EA\u30B9\u30C8\u672A\u767B\u9332</span>"}
          </div>
        </div>
      </div>
    </div>
  ...
  `}function I(a){let t=a.filter(u=>u.inRange),i=t.filter(u=>u.value>0),s=t.reduce((u,h)=>u+h.value,0),d=Math.max(0,...t.map(u=>u.value)),r=t[0]?.date,o=t[t.length-1]?.date,n=r&&o?`${$(r)} - ${$(o)}`:"\u2014",e=[],c=new Map;for(let u of t){let h=`${u.date.getFullYear()}-${String(u.date.getMonth()+1).padStart(2,"0")}`;if(!c.has(h)){let b={key:h,date:u.date,cells:[]};c.set(h,b),e.push(b)}c.get(h).cells.push(u)}let m=e.map(u=>{let h=u.cells.reduce((v,Y)=>v+Y.value,0),b=u.cells.filter(v=>v.value>0).length,_=Math.max(0,...u.cells.map(v=>v.value)),w=[...u.cells];for(;w.length<31;)w.push(null);let T=w.map(v=>v?`
      <span class="heatmap-day ${D(v.value)}" title="${v.iso}: ${v.value}\u66F2">
        <span>${v.date.getDate()}</span>
      </span>
    `:'<span class="heatmap-day is-empty" aria-hidden="true"></span>').join("");return`
      <section class="heatmap-month-card" aria-label="${$(u.date)} ${h}\u66F2 ${b}\u65E5\u914D\u4FE1">
        <div class="heatmap-month-head">
          <span>${$(u.date).replace(/^\d{4}\//,"")}</span>
          <strong>${h}</strong>
        </div>
