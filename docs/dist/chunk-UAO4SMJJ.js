import{d as f,e as h}from"./chunk-AW5KTZNU.js";import{B as L,C,D as $,G as k,M as y,a as g,c as v,p as M,q as D,r as H}from"./chunk-3BNHFXWM.js";function O(){let{songs:a,streams:t}=h.data,r=t.slice(0,5),n=f(),d=g("#panel-dashboard"),i=H(t,n);d.innerHTML=`
  ...
  `,F()}var j="sh1an-watch-history-v1";function x(){try{return JSON.parse(localStorage.getItem(j)||"[]")}catch{return[]}}function R(a){let t=Math.max(0,Math.floor(a)),r=Math.floor(t/3600),n=Math.floor(t%3600/60),d=t%60;return r>0?`${r}:${String(n).padStart(2,"0")}:${String(d).padStart(2,"0")}`:`${n}:${String(d).padStart(2,"0")}`}function B(){let a=x().slice(0,6);return`
    ...
    </div>`}function F(){let a=g("#dashboard-resume-list");a&&(a.onclick=n=>{let d=n.target.closest("[data-resume-idx]");if(!d)return;let i=x()[Number(d.dataset.resumeIdx)];if(!i?.url)return;let o=null;i.channel!=null&&i.index!=null&&(o=(h.channelData?.combined?.streams||h.data?.streams||[]).find(s=>s.channel===i.channel&&s.index===i.index)||null),window.__openStreamViewer?.(o||{url:i.url,title:i.title,isMv:!!i.isMv},i.t)});let t=g("#dashboard-resume-clear");t&&(t.onclick=()=>{try{localStorage.removeItem(j)}catch{}let n=g("#dashboard-resume-list");n&&(n.innerHTML='<div class="empty-state">\u518D\u751F\u5C65\u6B74\u306F\u307E\u3060\u3042\u308A\u307E\u305B\u3093</div>')});let r=g("#dashboard-resume-queue");r&&(r.onclick=()=>{let n=x(),d=h.channelData?.combined?.streams||h.data?.streams||[],i=n.map((o,e)=>{let s=o.channel!=null&&o.index!=null?d.find(l=>l.channel===o.channel&&l.index===o.index):null;return s?.url?{kind:"stream",key:`${s.channel}:${s.index}`,stream:s}:o.url?{kind:"mv",key:`history:${e}`,video:{url:o.url,title:o.title||"\u52D5\u753B",isMv:!!o.isMv}}:null}).filter(Boolean);i.length&&window.__playMyListInViewer?.({name:"\u8996\u8074\u5C65\u6B74",items:i,idx:0})})}function N(a,t,r){let n=t.filter(e=>e.daysSinceLast>=180).sort((e,s)=>s.count-e.count).slice(0,5),d=t.filter(e=>e.daysSinceLast!=null&&e.daysSinceLast<=30).sort((e,s)=>s.count-e.count).slice(0,5),i=M(a,"month",f()),o=M(a,"year",f());return`
