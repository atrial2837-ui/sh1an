import{d as f,e as h}from"./chunk-AW5KTZNU.js";import{B as H,C as L,D as $,G as M,M as y,a as g,c as v,p as w,q as x,r as D}from"./chunk-3BNHFXWM.js";function O(){let{songs:a,streams:t}=h.data,r=t.slice(0,5),n=f(),d=g("#panel-dashboard"),i=D(t,n);d.innerHTML=`
  ...
  `,F()}var j="sh1an-watch-history-v1";function k(){try{return JSON.parse(localStorage.getItem(j)||"[]")}catch{return[]}}function R(a){let t=Math.max(0,Math.floor(a)),r=Math.floor(t/3600),n=Math.floor(t%3600/60),d=t%60;return r>0?`${r}:${String(n).padStart(2,"0")}:${String(d).padStart(2,"0")}`:`${n}:${String(d).padStart(2,"0")}`}function B(){let a=k().slice(0,6);return`
    <div class="card dashboard-card dashboard-resume-card">
      ...
      <div class="dashboard-resume-list" id="dashboard-resume-list">
        ${a.length?a.map((t,r)=>{let n=M(t.url),d=Math.floor((Date.now()-(t.updatedAt||0))/864e5),i=d<=0?"\u4ECA\u65E5":`${d}\u65E5\u524D`;return`
    ...
    </div>`}function F(){let a=g("#dashboard-resume-list");a&&(a.onclick=n=>{let d=n.target.closest("[data-resume-idx]");if(!d)return;let i=k()[Number(d.dataset.resumeIdx)];if(!i?.url)return;let o=null;i.channel!=null&&i.index!=null&&(o=(h.channelData?.combined?.streams||h.data?.streams||[]).find(s=>s.channel===i.channel&&s.index===i.index)||null),window.__openStreamViewer?.(o||{url:i.url,title:i.title,isMv:!!i.isMv},i.t)});let t=g("#dashboard-resume-clear");t&&(t.onclick=()=>{try{localStorage.removeItem(j)}catch{}let n=g("#dashboard-resume-list");n&&(n.innerHTML='<div class="empty-state">\u518D\u751F\u5C65\u6B74\u306F\u307E\u3060\u3042\u308A\u307E\u305B\u3093</div>')});let r=g("#dashboard-resume-queue");r&&(r.onclick=()=>{let n=k(),d=h.channelData?.combined?.streams||h.data?.streams||[],i=n.map((o,e)=>{let s=o.channel!=null&&o.index!=null?d.find(l=>l.channel===o.channel&&l.index===o.index):null;return s?.url?{kind:"stream",key:`${s.channel}:${s.index}`,stream:s}:o.url?{kind:"mv",key:`history:${e}`,video:{url:o.url,title:o.title||"\u52D5\u753B",isMv:!!o.isMv}}:null}).filter(Boolean);i.length&&window.__playMyListInViewer?.({name:"\u8996\u8074\u5C65\u6B74",items:i,idx:0})})}function N(a,t,r){let n=t.filter(e=>e.daysSinceLast>=180).sort((e,s)=>s.count-e.count).slice(0,5),d=t.filter(e=>e.daysSinceLast!=null&&e.daysSinceLast<=30).sort((e,s)=>s.count-e.count).slice(0,5),i=w(a,"month",f()),o=w(a,"year",f());return`
  ...
  `}function P(a){if(!a)return`
    ...
    `;let t=a.url?M(a.url):"",r=(a.songs||[]).slice(0,15);return`
    <div class="card dashboard-card dashboard-latest-card">
      <div class="card-title">\u6700\u65B0\u306E\u6B4C\u67A0</div>
      <div class="latest-stream-log">
        ...
        <div class="latest-stream-body">
          <h3>${v(a.title||"\u914D\u4FE1")}</h3>
          <p>${y("mic")} ${(a.songs||[]).length}\u66F2 \u30FB ${H(a.date)}\u65E5\u524D</p>
  ...
  `}function I(a){let t=a.filter(c=>c.inRange),r=t.filter(c=>c.value>0),n=t.reduce((c,p)=>c+p.value,0),d=Math.max(0,...t.map(c=>c.value)),i=t[0]?.date,o=t[t.length-1]?.date,e=i&&o?`${$(i)} - ${$(o)}`:"\u2014",s=[],l=new Map;for(let c of t){let p=`${c.date.getFullYear()}-${String(c.date.getMonth()+1).padStart(2,"0")}`;if(!l.has(p)){let b={key:p,date:c.date,cells:[]};l.set(p,b),s.push(b)}l.get(p).cells.push(c)}let m=s.map(c=>{let p=c.cells.reduce((u,Y)=>u+Y.value,0),b=c.cells.filter(u=>u.value>0).length,C=Math.max(0,...c.cells.map(u=>u.value)),_=["\u65E5","\u6708","\u706B","\u6C34","\u6728","\u91D1","\u571F"].map(u=>`<span>${u}</span>`).join(""),T=c.cells.map(u=>`
      <span class="heatmap-day ${x(u.value)}" title="${u.iso}: ${u.value}\u66F2">
        <span>${u.date.getDate()}</span>
      </span>
    `).join("");return`
      <section class="heatmap-month-card" aria-label="${$(c.date)} ${p}\u66F2 ${b}\u65E5\u914D\u4FE1">
        ...
        <div class="heatmap-weekdays" aria-hidden="true">${_}</div>
