...
function _svInitShareModal() {
  ...
  const rebuild = () => {
    ...
    const text = st.title ? `${st.title}` : 'sh1an 歌唱データベース';
    const x = $('#sv-share-x');
...
const SV_SETLIST_COLLAPSED_KEY = 'sh1anViewerSetlistCollapsed';
...
function _svTsKey(stream) {
  return `sh1an-ts-${stream.channel || ''}-${stream.index || ''}`;
}
...
function _getPlaylists() {
  try { return JSON.parse(localStorage.getItem('sh1an-playlists') || 'null') || []; }
  catch (_) { return []; }
}

function _savePlaylists(pls) {
  try { localStorage.setItem('sh1an-playlists', JSON.stringify(pls)); } catch (_) {}
}
...
const SV_QUEUE_COLLAPSED_KEY = 'sh1anViewerQueueCollapsed';
...
window.__playMyListInViewer = (queue) => {
  ...
  _svListQueue = {
    ...
    items: queue.items,
    idx: 0,
    repeat: localStorage.getItem('sh1anListRepeat') === '1',
...
function _svHandleQueueClick(e) {
  ...
  if (btn.dataset.svqAction === 'repeat') {
    ...
    try { localStorage.setItem('sh1anListRepeat', _svListQueue.repeat ? '1' : '0'); } catch (_) {}
...
const CH_INFO = {
  new: {
    name: 'sh1an | シアン',
    handle: '@sh1an',
    url: 'https://www.youtube.com/@sh1an',
    label: 'Main',
    desc: 'ぽんこつ不憫な魔導士Vsinger\nきみたちの日常をじわじわ温かくする、一世紀推せる活動者を目指して邁進中。\n総合タグ #sh1arch1ve / ファンアート #sh1anotype / ファンネーム sh1anchu・しあんちゅ',
    links: [
      { icon: '<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>', label: 'X', url: 'https://x.com/sh1anos' },
      { icon: '<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M21.58 7.17a2.51 2.51 0 0 0-1.77-1.77C18.25 5 12 5 12 5s-6.25 0-7.81.4A2.51 2.51 0 0 0 2.42 7.17 26.9 26.9 0 0 0 2 12a26.9 26.9 0 0 0 .42 4.83 2.51 2.51 0 0 0 1.77 1.77c1.56.4 7.81.4 7.81.4s6.25 0 7.81-.4a2.51 2.51 0 0 0 1.77-1.77A26.9 26.9 0 0 0 22 12a26.9 26.9 0 0 0-.42-4.83ZM10 15.43V8.57L16 12l-6 3.43Z"/></svg>', label: 'YouTube', url: 'https://www.youtube.com/@sh1an' },
      { icon: '<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 3v10.55c-.59-.34-1.27-.55-2-.55-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4V7h4V3h-6z"/></svg>', label: 'Twitch', url: 'https://twitch.tv/sh1anos' },
      { icon: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M2 12h20"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10A15.3 15.3 0 0 1 12 2z"/></svg>', label: 'HP', url: 'https://lit.link/sh1an' },
      { icon: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="5"/><circle cx="12" cy="12" r="4"/><circle cx="17.5" cy="6.5" r="1"/></svg>', label: 'Instagram', url: 'https://instagram.com/sh1anos' },
    ],
    avatarUrl: 'https://yt3.googleusercontent.com/Dr_8IrPp_iU_7lBmTH8Z9r3Z1cU1QQcb7AwwKEJoURu7nVqa7i6MQ30Wr0feOi7AfGHPMzdjgQ=s900-c-k-c0x00ffffff-no-rj',
    bannerUrl: 'https://yt3.googleusercontent.com/JM4090e_TUtyfV4EBdfECwo3rzveeI3WgbO5VZHvw-2VySPqu2R9Q92NeYKYmhBmHmFQwHcYhw=w1707-fcrop64=1,00005a57ffffa5a8-k-c0xffffffff-no-nd-rj',
  },
};
...
function initWelcomeTip() {
  ...
  if (localStorage.getItem('sh1an-welcome-tip-dismissed') === '1') return;
  ...
  close.addEventListener('click', () => {
    tip.hidden = true;
    localStorage.setItem('sh1an-welcome-tip-dismissed', '1');
  });
}
